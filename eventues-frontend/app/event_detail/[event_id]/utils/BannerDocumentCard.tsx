// components/OrganizatorEventDetails/BannerDocumentCard.tsx
"use client";
...
const BannerDocumentCard: React.FC<BannerDocumentCardProps> = ({
  eventId,
  onNotify,
  onUpdate,
}) => {
      ...
      if (response.status === 200) {
        ...
        if (data.length > 0) {
          existingFiles = data.map((file) => ({
            ...
            title:
              file.title ||
              file.file_name.split(".")[0].replace(/_/g, " "),
            required:
  ...
  const updateFileTitleInBackend = async (fileData: FileData) => {
    try {
      ...
      setErrors((prev) => [
        ...prev,
        "Erro ao atualizar título do documento.",
      ]);
                    ...
                    <StyledTableCell>
                      <TextField
                        ...
                        error={!!errors.find((error) =>
                          error.includes("Título do documento")
                        )}
