// components/EconomyCalculator.tsx
'use client';
...
const CalculatorCard = styled(Card)(({ theme }) => ({
  ...
  [theme.breakpoints.up('md')]: {
    padding: theme.spacing(4), // Increase padding on medium screens
  },
  '&:hover': {
  ...
  useEffect(() => {
    const calculateSavings = (price: number, quantity: number): { totalSavings: number; participantPayment: number } => {
      ...
      if (price <= 20) {
        ...
        participantFeePerTicket = price + 1.99; // Participant payment value
      } else {
        eventuesFeePerTicket = 0.0799 * price;
        competitorFeePerTicket = 0.10 * price;
        participantFeePerTicket = price + (0.0799 * price); // Participant payment value
      }
